...
$id = $emaildb;
$sql = "SELECT * FROM `billing_users` WHERE `user_id` = '$id'";
$result = mysqli_query($conn, $sql);
...
if (mysqli_num_rows($result) > 0) {
  // output data of each row
  while($row = mysqli_fetch_assoc($result)) {
    $resultssearch = $row["balance"]. "<br>";
  }
} else {
  $resultssearch = "Sorry, we couldn't find your Email!";
}
?>
<html>
      ...
      <div class="service_container">
        <div class="box active">
          ...
          <div class="detail-box">
            <h5>
              <strong><?php echo $resultssearch ?></strong>
            </h5>
          </div>
        </div>
      </div>
      <div class="btn-box">
        <a href="https://panel.nexussociety.net">
          I'd like to use those now!
        </a>
      </div>
    </div>
  </section>
